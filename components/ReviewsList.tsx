  ...
  return (
    <div className="space-y-6">
      ...
      {reviews.map((review) => {
        ...
        return (
          <div key={review.id} className="card p-6 border-l-4 border-l-primary-600 hover:border-l-primary-500 transition-all bg-white shadow-sm border border-neutral-200">
              ...
              <div className="grid md:grid-cols-2 gap-6 bg-neutral-50 p-5 rounded-xl border border-neutral-200">
                ...
                <div className="space-y-4 text-base">
                  ...
                  <div className="grid grid-cols-[130px_30px_1fr] gap-y-3 items-center">
                    ...
                    {/* Notlandırma */}
                    {(review.grading_fairness || 0) > 0 && (
                      <>
                        <>
                          <span className="text-neutral-600">{t('grading_fairness')}</span>
                          <span className="font-bold text-neutral-900 text-center">{review.grading_fairness}</span>
                          <span className="text-sm text-neutral-500 ml-2 truncate">
                            {METRIC_DESCRIPTIONS.grading_fairness[(review.grading_fairness || 0) as 1 | 5] ? `(${tConstants(`metrics.${METRIC_DESCRIPTIONS.grading_fairness[(review.grading_fairness || 0) as 1 | 5]}`)})` : ''}
                          </span>
                        </>
                      </>
                    )}
