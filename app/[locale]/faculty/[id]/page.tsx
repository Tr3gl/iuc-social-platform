...
export default function FacultyPage() {
  ...
  // Sort state - default to credits now
  const [sortBy, setSortBy] = useState<'name' | 'code' | 'credits'>('credits');
  ...
  const filteredCourses = useMemo(() => {
    let result = courses.filter(course => {
      ...
      return matchesSearch && matchesCourseType && matchesClassType && matchesSemester && matchesInstructor;
    });
    ...
    return result;
  }, [courses, searchQuery, filters, sortBy, sortOrder, locale]);
  ...
  const clearFilters = () => {
    setFilters({ courseType: '', classType: '', semester: '', instructor: '' });
    setSearchQuery('');
  };
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
            {childDepartments.map((dept: any) => (
              <button
                ...
                className="card p-6 text-left hover:shadow-lg transition-all duration-200 group border-2 border-neutral-200 bg-white hover:border-primary-400"
              >
