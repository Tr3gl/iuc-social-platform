...
// checkAdminPassword removed, use verifyAdminStatus below instead.
...
export async function approvePendingTag(
  ...
  if (!duplicate) {
    ...
    if (insertError) {
      ...
      throw new Error(insertError.message || 'Failed to create tag in system');
    }
  }
...
export async function createTag(name: string, type: 'positive' | 'negative') {
  ...
  const { error } = await supabaseAdmin
    .from('tags')
    .insert({ name, type, is_verified: true, created_by: null } as any); // created_by is dynamically null since it's an admin
...
export async function getAllFaculties() {
  ...
  if (error) throw new Error(error.message || 'Failed to fetch faculties');
  return data ?? [];
}

export async function createCourse(data: { code: string; name: string; faculty_id: string }) {
  ...
  if (error) throw new Error(error.message || 'Failed to create course');
  revalidatePath('/admin');
  return { success: true };
}

export async function updateCourse(id: string, data: { code: string; name: string; faculty_id: string }) {
  const isAdmin = await verifyAdminStatus();
  if (!isAdmin) throw new Error("Unauthorized");
  ...
  if (error) throw new Error(error.message || 'Failed to update course');
  revalidatePath('/admin');
  return { success: true };
}
...
export async function deleteCourse(id: string) {
  ...
  // First cascade delete any dependent logic if necessary (though foreign keys might handle this)
  // For safety, we just allow Supabase FK constraints to handle ON DELETE CASCADE, or it will throw.
