-- ============================================================================
-- MIGRATION: Fix get_course_stats Aggregation Logic
-- ============================================================================
...
BEGIN
    ...
    IF r_total = 0 THEN
        RETURN QUERY SELECT
            0::BIGINT, NULL::NUMERIC, NULL::NUMERIC, NULL::NUMERIC, NULL::NUMERIC,
    ...
    -- 2. Distributions (Computed via independent queries to avoid join explosion)
    ...
    SELECT jsonb_object_agg(difficulty_value_alignment, c) INTO r_dva_counts FROM (SELECT difficulty_value_alignment, COUNT(*) as c FROM reviews WHERE course_id = course_uuid AND is_hidden = false GROUP BY difficulty_value_alignment) t;
    SELECT jsonb_object_agg(exam_format, c) INTO r_ef_counts FROM (SELECT exam_format, COUNT(*) as c FROM reviews WHERE course_id = course_uuid AND is_hidden = false GROUP BY exam_format) t;
